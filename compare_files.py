...
def app(supplement_name: str, delete_enabled: bool, custom_chars: str):
    ...
    Zeigt Fortschritt und aktuellen Zeileneintrag.
    """
    ...
    def load_and_clean(file, name):
        raw = pd.read_excel(
            file, sheet_name=name, header=None, engine="openpyxl"
        )
        hdr = detect_header_row(raw)
        df = pd.read_excel(
            file, sheet_name=name, header=hdr, engine="openpyxl"
        )
    ...
    compare = [
        c for c in master_cols + measure_cols
